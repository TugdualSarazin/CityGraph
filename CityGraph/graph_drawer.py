...
class Drawer:
    ...
    def draw(self, agent_paths):
        ...
        for apath in agent_paths:
            ...
            self.graph_edge_feature(ax_graph_agent_weight, apath.agent_id)
    ...
    def graphs_agent_weight(self, fig, gs_elem, agent_type):
        ...
        for attr in EdgeType.weight_keys:
            ...
            self.graph_edge_feature(ax_graph_factor, attr, factor=factor, display_tile=False)
            # Increment row index
            cax += 1

        # ax_graph_agent = fig.add_subplot(inner_gs[1:, graph_col_size+1:])
        ax_graph_agent = fig.add_subplot(inner_gs[1:, 2])
        self.graph_edge_feature(ax_graph_agent, agent_type, display_tile=False)

    def draw_agents_paths(self, edges_num, _ax_edge_type, ax_map, ax_chart, apaths: [AgentPath]):
        #ax_edge_type.clear()
    ...
    def plot_agent_weight_path(self, ax, agents_path: [AgentPath], edges_num=None):
        for apath in agents_path:
            # Select only first numbers edges
            if edges_num:
    ...
    def plot_all_weight_path(self, ax, agent_path: AgentPath, edges_num=None):
        ...
        for attr in EdgeType.weight_keys:
            agent_factor = EdgeType.weight_config[attr]['agent_type'][agent_path.agent_id]
            attr_values = [self.graph.graph[u][v][attr] * agent_factor for u, v in path_edges]
    ...
    def graph_agent_path(self, ax, agents_path: [AgentPath], edges_num=None):
        for apath in agents_path:
            if edges_num:
                ...
                path_edges = apath.path_edges
            nx.draw_networkx_edges(self.graph.graph, self.graph.pos, ax=ax, edgelist=path_edges, edge_color=apath.color,
                                   width=self.edge_size + 1)
        nx.draw_networkx_edges(self.graph.graph, self.graph.pos, ax=ax, alpha=0.2)
        nx.draw_networkx_nodes(self.graph.graph, self.graph.pos, ax=ax, node_color=self.node_color,
    ...
    def draw_graphs_row(self, fig, gs, row):
        ...
        for attr in EdgeType.weight_keys:
            ...
            self.graph_edge_feature(ax, attr, cmap=plt.cm.coolwarm)
            rax += 1
    ...
    def graph_edge_type(self, ax):
        ...
        colors = [mapping_etype_color[etype] for etype in nx.get_edge_attributes(self.graph.graph, 'type').values()]

        # Draw edges and nodes
        ec = nx.draw_networkx_edges(self.graph.graph, self.graph.pos, ax=ax, width=self.edge_size,
                                    edge_color=colors, edge_cmap=cmap)
        nc = nx.draw_networkx_nodes(self.graph.graph, self.graph.pos, ax=ax, node_color=self.node_color,
                                    node_size=self.node_size)
    ...
    def graph_edge_feature(self, ax, feature: Indicator10, factor=1, display_tile=True, cmap=plt.cm.RdYlGn):
        # def graph_edge_attr(self, ax, attr, factor=1, display_tile=True, cmap=plt.cm.autumn.reversed()):
        # Extract attr values
        attr_values = [data.get(feature.key, 0) * factor for _, _, data in self.graph.graph.edges(data=True)]
    ...
    def graph_node_connection(self, ax):
        ...
        n_edges = [len(self.graph.graph.edges(n)) for n in self.graph.graph.nodes()]

        # Draw edges and nodes
        ec = nx.draw_networkx_edges(self.graph.graph, self.graph.pos, ax=ax, alpha=0.2)
        nc = nx.draw_networkx_nodes(self.graph.graph, self.graph.pos, ax=ax, node_size=self.node_size,
