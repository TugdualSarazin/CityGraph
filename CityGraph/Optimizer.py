...
class Agent:
    def __init__(self, agent_type: str, src_node, dest_type, color='red'):
    ...
    def best_paths(self, G):
        ...
        for target in targets:
            path = AgentPath(self.agent_type, G, self.src_node, target, self.color)
            if best_path is None or path.weight < best_path.weight:
                best_path = path
            print(path)
        return best_path
